...
build-gp-main:
	...
	docker run -v `pwd`/local:/local --user `id --user` quay.io/suikawiki/swfonts:swcf cp -a /app/fonts/swir /local/swir
	docker run -v `pwd`/local:/local --user `id --user` quay.io/suikawiki/swfonts:swcf cp -a /app/fonts/swcfk /local/swcfk
	docker run -v `pwd`/local:/local --user `id --user` quay.io/suikawiki/swir:data-main cp -a /app/indexes /local/swir-indexes
